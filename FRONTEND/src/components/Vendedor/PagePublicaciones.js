...
import Navbar from './navbarVendedor'
...
export function PagePublicaciones () {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!loaded) {
                const result = await axios.get('http://localhost:3001/productos/id/1')
                if (result.data) {
                    setDataLoaded(true)
    ...
    return (
        <div className="App">
            ...
            <ListaPublicaciones products={ProductsData} onAdd={addProduct} />
        </div>
    )
}