...
import PostsListComprador from "./PostsListComprador"
...
export function PageComprador () {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!loaded) {
                const result = await axios.get('http://localhost:3001/publicaciones')
                if (result.data) {
                    setDataLoaded(true)
    ...
    return (
        <div className="App">
            ...
            <PostsListComprador products={ProductsData} onAdd={addProduct} />
        </div>
    )
}